...
//		'ХИМИЧЕСКИЕ МАТЕРИАЛЫ',
	...
	public function __construct($feedPath, $categoriesPath, $utmMark,  Cutter $cutter, Replacer $replacer, $rbfGoodsPath, $addUtmMark = true)
	{
		$this->feedPath = $feedPath;
        $this->categoriesPath = $categoriesPath;
        $this->rbfGoodsPath = $rbfGoodsPath;
...
';
                ...
                echo 'RBF Price = '.$sku['price'][0] / 100 * intval(str_replace('%', '', $rbfGood['Discount']))."\n";
...
';
    ...
    public function getRBFGoods()
    {
        if (!$this->rbfGoods) {
            $this->rbfGoods = json_decode(file_get_contents($this->rbfGoodsPath), true);
        }

        return $this->rbfGoods;
    }
